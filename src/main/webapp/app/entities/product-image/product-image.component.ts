...
export class ProductImageComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.productImageService.query({
            page: this.page,
            size: this.itemsPerPage,
            sort: this.sort()
        }).subscribe(
            (res: HttpResponse<ProductImage[]>) => this.onSuccess(res.body, res.headers),
    ...
    registerChangeInProductImages() {
        this.eventSubscriber = this.eventManager.subscribe('productImageListModification', (response) => this.reset());
    }
